...
export function createServer() {
  const server = new McpServer(
    ...
    {
      title: "交互式电子表格",
      description: "在对话框中打开交互式电子表格编辑器",
    },
    {
      description: `将表格数据显示为交互式电子表格。当用户要求展示表格、对比数据、预算、报告或任何适合网格布局的结构化数据时使用。
...
也可以导入 Excel/CSV 文件：使用 import_spreadsheet 工具传入 Base64 编码的文件内容。`,
      inputSchema: {
    ...
    {
      title: "导入电子表格",
      description: "导入 Excel 或 CSV 文件并显示为交互式表格",
    },
    {
      description: `导入 Excel 或 CSV 文件。接收 Base64 编码的文件内容和文件类型（xlsx 或 csv）。
解析后将数据转换为 Skybridge 表格格式并显示在交互式编辑器中。`,
      inputSchema: {
